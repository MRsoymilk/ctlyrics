...
Linux 托盘优先使用 StatusNotifierItem，兼容 KDE Plasma 和启用 `tray` 模块的 Waybar；AwesomeWM 等 X11 环境自动回退到 XEmbed。右键菜单会显示当前歌曲、歌手、播放状态和进度，并提供上一首、播放/暂停、下一首控制；XEmbed 进度条支持点击跳转，歌曲信息超宽时自动滚动。左键可显示或隐藏置于普通窗口之上的当前歌词气泡，拖动气泡可在本次运行内调整并保留位置；在托盘或气泡上滚动鼠标滚轮可切换横向显示和字形保持正向、从上到下排列的纵向显示。Wayland 优先使用 Layer Shell 歌词窗口，不支持时回退到普通 Wayland 窗口，窗口初始化失败时再使用系统通知。GNOME Wayland 需要安装 AppIndicator/KStatusNotifier 扩展。托盘不可用时歌词界面仍可正常运行。
